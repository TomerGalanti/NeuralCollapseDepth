...
class ResNet(nn.Module):
    ...
    def __init__(self, settings, block, num_block):
        super().__init__()
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(self.num_input_channels, 64, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU())
    ...
    def forward(self, x):
        ...
        for i in range(self.top_depth):
            output = self.top_layers[i](output)
            embeddings += [output]
            output = self.relu(output)
...
def resnet18(settings):
    ...
    """
    return ResNet(settings, BasicBlock, [2, 2, 2, 2])

def resnet34(settings):
    """ return a ResNet 34 object
    """
    return ResNet(settings, BasicBlock, [3, 4, 6, 3])

def resnet50(settings):
    """ return a ResNet 50 object
    """
    return ResNet(settings, BottleNeck, [3, 4, 6, 3])

def resnet101(settings):
    """ return a ResNet 101 object
    """
    return ResNet(settings, BottleNeck, [3, 4, 23, 3])

def resnet152(settings):
    """ return a ResNet 152 object
    """
    return ResNet(settings, BottleNeck, [3, 8, 36, 3])